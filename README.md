...
## Purpose
PTL is designed to fix the following symptoms of primitive input file readers, each of which has been found in an existing, large-scale scientific computing code:
...
and needs to be run again! With a `PTL::PropertyTree input` object, simply call `input.DebugPrint()` and everything is there.
...
improperly formatted). PTL will identify all unrecognized or improperly formatted inputs.
...
Using a `PTL::Interactive i` object, and a command-line argument `PTL:defaultVals`, simply call `i.Run()` to create a
...
You can't remember what `method=1`, `method=2`, etc. correspond to. Using a `PTL::Variables::PTLEnum` or `PTL::Variables::PTLAutoEnum`
object, options like this can have meaningful enumeration-style labels.

* Your input file options are not comprehensively documented. All PTL variables take a string description argument so that
every input option is documented. This is a huge help to rapidly bring newcomers onboard with your code!

* You have multiple separate submodules / objects / libraries that all have different input files. With `PTL::PropertyTree subInput, mainInput`
...
* You have multiple parameters in your input file that, for some reason, all have to have the same value. The `PTL::PreProcessContext` object is a part of every
`PTL::PropertyTree` (and is automatically active) and allows for contextualized `#define` directives, e.g. `#define myValue 0.1`, and then
`someValue = $(myValue)`
...
Any input type can be implemented by inheriting the `PTL::Variables::InputVariable` class. PTL currently has implementations for:

* `PTL::Variables::PTLInteger`: An integer value.
* `PTL::Variables::PTLBoolean`: A boolean value.
* `PTL::Variables::PTLDouble`: A double-precision value.
* `PTL::Variables::PTLString`: A string (`std::string`) value.
* `PTL::Variables::PTLEnum`: A string-based enumeration that maps to an integer value. Options are passed to the constructor as a colon-delimited list,
...
* `PTL::Variables::PTLAutoEnum`: An enumeration defined by an `enum` implementation, e.g. `enum myType{option1,option2}`. Since C++ is not reflective,
...
```
* `PTL::Variables::PTLDynamicDoubleArray`: An arbitrary-size array of double-precision values. This object maps to two outputs: an integer representing
...
* `PTL::Variables::PTLStaticDoubleArray`: A fixed-size array of double-precision values. This object maps only to a `double*`, and is assumed to have a
...
`PTL::Variables::PTLDynamicDoubleArray`.

* `PTL::Variables::PTLDynamicIntegerArray`: This object is identical to a `PTL::Variables::PTLDynamicDoubleArray`, just with an integer element type.
* `PTL::Variables::PTLStaticIntegerArray`: This object is identical to a `PTL::Variables::PTLStaticIntegerArray`, just with an integer element type.
