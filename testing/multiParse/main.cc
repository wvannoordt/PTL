...
int main(void)
{
	...
	= new PTL::Variables::PTLBoolean(false, "Flag to enable to wall model");

	input["NavierStokes"]["WallModel"]["outputWallModelData"].MapTo(&outputWallModelData)
	= new PTL::Variables::PTLBoolean(false, "Flag to enable wall model output");

	input["NavierStokes"]["WallModel"]["wallModelOutputDataInterval"].MapTo(&wallModelOutputDataInterval)
	= new PTL::Variables::PTLInteger(100, "Controls the interval at which wall model data is written");

	input["NavierStokes"]["WallModel"]["forcingOffset"].MapTo(&forcingOffset)
	= new PTL::Variables::PTLInteger(4, "Wall model sampling location (number of cells)");

	input["NavierStokes"]["WallModel"]["rayDim"].MapTo(&rayDim)
	= new PTL::Variables::PTLInteger(30, "Number of mesh points in wall-normal direction in the wall model");

	input["NavierStokes"]["WallModel"]["errorTolerance"].MapTo(&errorTolerance)
	= new PTL::Variables::PTLDouble(1e-6, "Generic error tolerance for iterative procedures in the wall model numerics");

	input["NavierStokes"]["WallModel"]["outputDir"].MapTo(&wallModelOutputDir)
	= new PTL::Variables::PTLString(".", "Location for wall model output data");

	input["NavierStokes"]["WallModel"]["SampleXLocations"].MapTo(&sampleXLocations, &numSampleXLocations)
	= new PTL::Variables::PTLDynamicDoubleArray("X-locations for inner-solution sampling with precision 1e-5");

	input["NavierStokes"]["WallModel"]["Momentum"]["equation"].MapTo(&momEquation)
	= new PTL::Variables::PTLEnum("ODE", "ODE:allmaras:linear", "Specifies what kind of equation to solve for wall model momentum equation");

	input["NavierStokes"]["WallModel"]["Momentum"]["underRelaxation"].MapTo(&momEqUnderRelax)
	= new PTL::Variables::PTLDouble(0.4, "Under-relaxation factor for momentum equation Newton solve");

	input["NavierStokes"]["WallModel"]["Turbulence"]["equation"].MapTo(&turbEquation)
	= new PTL::Variables::PTLEnum("vanDriest", "ODE:vanDriest:linear", "Specifies what kind of equation to solve for wall model turbulence equation");

	input["NavierStokes"]["WallModel"]["Turbulence"]["underRelaxation"].MapTo(&turbEqUnderRelax)
	= new PTL::Variables::PTLDouble(0.4, "Under-relaxation factor for turbulence equation Newton solve");

	input["NavierStokes"]["WallModel"]["Energy"]["equation"].MapTo(&engyEquation)
	= new PTL::Variables::PTLEnum("ODE", "ODE:coroccoBuseman:linear", "Specifies what kind of equation to solve for wall model energy equation");
